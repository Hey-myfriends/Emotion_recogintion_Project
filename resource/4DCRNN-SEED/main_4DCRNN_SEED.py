...
falx = np.load('/Users/anayisi/Documents/Master/Emotion recognition/SEED/SEED/Preprocessed_EEG/t6x_89.npy')
#falx = np.load('/Volumes/Données/Preprocessed_EEG/t6x_89.npy')
y = np.load('/Users/anayisi/Documents/Master/Emotion recognition/SEED/SEED/Preprocessed_EEG/t6y_89.npy')
#y = np.load('/Volumes/Données/Preprocessed_EEG/t6y_89.npy')
# one_y = np.array([y[:1697]] * 3).reshape((-1,))
# one_y = to_categorical(one_y, num_classes)
print(y.shape)
one_y_1 = np.array([y[:1126]] * 3).reshape((-1,))#这是把原来的复制三份吗
print(one_y_1.shape)
...
for nb in range(15):
    ...
    # create model
    for train, test in kfold.split(one_falx, one_y.argmax(1)):#这个.argmax（）把标签转化成了0 1 2
        print(kfold.split(one_falx, one_y.argmax(1)))
        img_size = (img_rows, img_cols, num_chan)
        print(one_y.argmax(1).shape)
        ...
        log_dir = "/Users/anayisi/Downloads/4DCRNN/" + datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
        ...
        model.fit([x_train[:, 0], x_train[:, 1], x_train[:, 2], x_train[:, 3], x_train[:, 4], x_train[:, 5]], y_train, epochs=100, batch_size=128, verbose=0, callbacks=[tensorboard_callback])
        # evaluate the model
