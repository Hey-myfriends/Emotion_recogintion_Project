...
def decompose(file, name):
    ...
    for trial in range(15):
        ...
        #label原来是一个空数组，然后往里面添加值SEED_1D_X89.py。。为原始array添加一些value（arr,values,axis=none)
        ...
        for channel in range(62):
            ...
            #竖着把array垒起来，现在得到了五个叠起来的一维数组。
        #这里-1的意思是无论是几，只要和后面两个数兼容就行。
        temp_trial_de = temp_de.reshape(-1, 5, num_sample)
        #transpose（【0，1，2】）不起变化，0。1。2代表的是矩阵的三个维度，括弧里面的代表变化后的维度。所以现在变成（num_sample,x,5)
...
#file_path = '/Données/Preprocessed_EEG/'
...
np.save("/Users/anayisi/Documents/Master/Emotion recognition/SEED/SEED/Preprocessed_EEG/X_1D.npy", X)
np.save("/Users/anayisi/Documents/Master/Emotion recognition/SEED/SEED/Preprocessed_EEG/y.npy", y)




X = np.load('/Users/anayisi/Documents/Master/Emotion recognition/SEED/SEED/Preprocessed_EEG/X_1D.npy')
y = np.load('/Users/anayisi/Documents/Master/Emotion recognition/SEED/SEED/Preprocessed_EEG/y.npy')
# 生成8*9的矩阵形式
...
X89[:, 6, 1:8, :] = X[:, 50:57, :]
X89[:, 7, 2:7, :] = X[:, 57:62, :]
np.save("/Users/anayisi/Documents/Master/Emotion recognition/SEED/SEED/Preprocessed_EEG/X89.npy", X89)
