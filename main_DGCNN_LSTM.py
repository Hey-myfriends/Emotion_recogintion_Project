...
class Arguments(object):
    def __init__(self, log_=True) -> None:
        ...
        self.lr_drop = 15
        self.gamma = 0.5
        self.tag = "total_epochs_{}_DGCNN_LSTM2".format(self.epochs)
    ...
    def log(self):
        logger.info(f"epoches: {self.epochs}, bs: {self.batchsize}, remove the relu of final classifier")
...
def main():
    ...
    logger.info(f"in_chan: {in_chan}, out_chan: {out_chan}, d: {d}, num_class: {num_cls}, lstm_hid_size: {hidden_size}")

    dataset_train, dataset_val = build_dataset(feat_to_4D=False)
    ...
    model, criterion = build_model2(in_chan, K, out_chan, d, num_cls, hidden_size, cls_weights)
    logger.info("Model: \n{}".format(str(model)))
